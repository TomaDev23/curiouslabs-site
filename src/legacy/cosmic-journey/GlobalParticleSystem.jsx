...
export default function GlobalParticleSystem({ 
  ...
  const initializeStarField = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Star colors - Create different star types
      ...
      // Size variation - occasional larger stars
      sizes[i] = (Math.random() > 0.99) ? 6.0 + Math.random() * 3.0 : 
                 (Math.random() > 0.95) ? 3.0 + Math.random() * 2.0 : 
                 1.0 + Math.random() * 1.5;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          float twinkleSpeed = 1.5 + distanceFactor * 0.5;
          float twinkleAmount = 0.3 + cosmicIntensity * 0.2;
          float twinkle = sin(time * twinkleSpeed + randomOffset) * twinkleAmount + (1.0 - twinkleAmount);
          
          // Cosmic scene enhancement - stars move slightly
          vec3 modPosition = position;
          if (cosmicIntensity > 0.0) {
            // Add subtle motion to closer stars in cosmic scenes
            float motionScale = (1.0 - distanceFactor) * cosmicIntensity * 0.5;
            modPosition.x += sin(time * 0.2 + randomOffset) * motionScale;
            modPosition.y += cos(time * 0.3 + randomOffset * 1.4) * motionScale;
          }
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          alpha = max(alpha, 1.0 - smoothstep(0.3, 1.0, radius) * 0.8 * vOpacity);
  ...
  const updateStarField = (time, delta) => {
    ...
    let cosmicIntensity = 0.2; // Base cosmic effect intensity
    ...
    switch (activeScene) {
      case 'dormant':
        ...
        starOpacity = 0.4 + (sceneProgress * 0.3); // Increased from 0.2
        starDensity = 0.5 + (sceneProgress * 0.2); // Increased from 0.3
        baseSize = 1.5; // Added base size
        break;
        
      case 'awakening':
        // Stars become gradually more visible
        starOpacity = 0.7 + (sceneProgress * 0.3); // Increased from 0.5
        starDensity = 0.7 + (sceneProgress * 0.3); // Increased from 0.5
        cosmicIntensity = 0.2 + sceneProgress * 0.4; // Increased from sceneProgress * 0.3
        baseSize = 1.8; // Increased
        break;
        
      case 'cosmicReveal':
        // Stars are prominent and have cosmic effects
        starOpacity = 0.9; // Increased from 0.8
        starDensity = 0.9 + (sceneProgress * 0.1); // Increased from 0.8
        cosmicIntensity = 0.4 + (sceneProgress * 0.4); // Increased from 0.3
        baseSize = 2.0 + (sceneProgress * 0.3); // Increased from 1.1
        break;
      ...
      case 'cosmicFlight':
        ...
        starOpacity = 1.0;
        starDensity = 1.0;
        cosmicIntensity = 0.8 + (sceneProgress * 0.2); // Increased from 0.6
        baseSize = 2.2; // Increased from 1.3
        break;
        
      case 'sunApproach':
        // Stars start to fade as sun dominates
        starOpacity = 1.0 - (sceneProgress * 0.4); // Decreased fade rate from 0.5
        starDensity = 1.0;
        cosmicIntensity = 1.0 - (sceneProgress * 0.4); // Decreased fade rate from 0.5
        baseSize = 2.0 - (sceneProgress * 0.3); // Increased base from 1.3
        break;
        
      case 'sunLanding':
        // Minimal stars as sun overwhelms
        starOpacity = 0.6 - (sceneProgress * 0.5); // Increased from 0.5
        starDensity = 0.8 - (sceneProgress * 0.4);
        cosmicIntensity = 0.5 - (sceneProgress * 0.5);
        baseSize = 1.5; // Increased from 1.0
        break;
    ...
    // Apply smooth transitions to the uniforms
    ...
    // Camera effects based on global scroll
    if (cameraRef.current) {
      // Subtle camera movement to enhance immersion
      ...
      if (activeScene === 'cosmicFlight' || activeScene === 'cosmicReveal') {
        const intensity = activeScene === 'cosmicFlight' ? 0.3 : 0.15;
        cameraRef.current.rotation.z = Math.sin(time * 0.1) * 0.02 * intensity;
      } else {
